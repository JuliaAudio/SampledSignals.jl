...
@testset "SampleBuf Tests" begin
    ...
    @testset "Invalid units throw an error" begin
        ...
        buf = SampleBuf(arr, TEST_SR);
        @test_throws ErrorException buf[1Hz]
    end
    ...
    @testset "SampleBufs can be indexed in Hz" begin
        N = 512
        arr = rand(TEST_T, N, 2)
        buf = SampleBuf(arr, N / TEST_SR,:freq)
    ...
    @testset "FFT of SampleBufs" begin
        arr = rand(TEST_T, 512)
        buf = SampleBuf(arr, TEST_SR)
        spec = FFTW.fft(buf)
        @test isa(usamplerate(spec),Unitful.Time)
        @test eltype(spec) == Complex{TEST_T}
        @test samplerate(spec) == 512/TEST_SR
        @test nchannels(spec) == 1
        @test spec == SampleBuf(FFTW.fft(arr), 512/TEST_SR,:freq)
        buf2 = FFTW.ifft(spec)
        ...
        @test all(isapprox(buf2, buf))
    end

    @testset "1D SampleBufs can be convolved" begin
        arr1 = rand(TEST_T, 8)
        arr2 = rand(TEST_T, 10)
        result = SampleBuf(conv(arr1, arr2), TEST_SR)
        @test conv(SampleBuf(arr1, TEST_SR), SampleBuf(arr2, TEST_SR)) == result
        @test conv(SampleBuf(arr1, TEST_SR), arr2) == result
        @test conv(arr1, SampleBuf(arr2, TEST_SR)) == result
    end

    @testset "2D SampleBufs can be convolved" begin
...
#     TODO: replace with new interface tests
...
#     end
    @testset "buf displays data with NaN values." begin
        # TODO
    end
    ...
    @testset "multichannel buf prints prettily" begin
        ...
        expected = """300-frame, 2-channel, Float64 buffer.
0.00625 s sampled at 48000.0 Hz
▆▆▆▆▆▆▆▆▆▆▅▅▅▅▅▅▄▄▃▃▄▄▅▅▅▅▅▅▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▅▅▅▅▅▅▄▄▃▃▄▄▅▅▅▅▅▅▆▆▆▆▆▆▆▆▆▆
▃▄▄▅▅▅▅▅▅▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▅▅▅▅▅▅▄▄▂▄▄▅▅▅▅▅▅▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▅▅▅▅▅▅▄▄▃"""
        iobuf = IOBuffer()
    ...
    @testset "1D buf prints prettily" begin
        ...
        expected = """300-frame, 1-channel, Float64 buffer.
0.00625 s sampled at 48000.0 Hz
▆▆▆▆▆▆▆▆▆▆▅▅▅▅▅▅▄▄▃▃▄▄▅▅▅▅▅▅▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▅▅▅▅▅▅▄▄▃▃▄▄▅▅▅▅▅▅▆▆▆▆▆▆▆▆▆▆"""
        iobuf = IOBuffer()
        display(TextDisplay(iobuf), buf)
    ...
    @testset "zero-length buf prints prettily" begin
        ...
        expected = """0-frame, 2-channel, Float64 buffer.
0.0 s sampled at 48000.0 Hz"""
        iobuf = IOBuffer()
        display(TextDisplay(iobuf), buf)
        @test String(take!(iobuf)) == expected
    end
